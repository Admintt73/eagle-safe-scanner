...
# 创建安全目录
mkdir -p "$WORK_DIR" "$REPORT_DIR" "$DB_DIR" 2>/dev/null
...
check_processes() {
    ...
    if command -v ps &> /dev/null; then
        if ps --version 2>&1 | grep -q "busybox"; then
...
system_info() {
    ...
    if command -v free &> /dev/null; then
        if free --version 2>&1 | grep -q "busybox"; then
...
main() {
    ...
    while true; do
        ...
        # 修复Termux输入问题：强制读取单个字符
        read -n 1 -p "请选择 (0-9): " choice
        echo
